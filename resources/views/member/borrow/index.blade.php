    ...
    <main class="bg-white py-6" x-data="borrowModal()">
        ...
        <div x-show="showBorrowModal" class="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-auto">
            <div class="bg-white rounded-2xl w-full max-w-6xl mt-10 p-6 relative">
                ...
                <h2 class="text-xl font-bold text-sky-800 mb-4">Cari Buku</h2>

                <!-- Input search -->
                <input type="text" x-model="query" @input.debounce.100ms="searchBooks" placeholder="Cari buku..."
                    ...
                    <template x-for="book in books" :key="book.id">
                        <a :href="`{{ route('member.collection.show', ['book' => 'BOOK_ID']) }}`.replace('BOOK_ID', book.id)"
                            ...
                            <div class="p-3">
                                ...
                                <p class="text-xs font-medium text-sky-700" x-text="'Penulis: ' + book.author"></p>
                            </div>
                        </a>
                    </template>

                    <template x-if="books.length === 0 && query.length >= 2">
                        <div class="col-span-full text-center text-gray-500">Tidak ada buku ditemukan.</div>
                    </template>
                </div>
            </div>
        </div>
